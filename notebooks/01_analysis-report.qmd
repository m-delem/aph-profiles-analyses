...
The pipeline code below it runs all of these functions sequentially, 
sometimes saving heavy computations along the way. 
...
```{r}
#| label: add-import
#| add-from: ../R/R/03_model/build_behav_rdm.R
#| code-summary: Behavioural RDM function
#| echo: true
...
df |> 
  ...
  reframe(
    n = n(),
    across(c(vviq, osivq_s, osivq_v, score_raven, span_digit.x, score_sri), ~mean(.) |> round(2)))
```
:::
...
df |> 
  scale_reduce_vars() |>
  add_cluster_vars(df_raw = df, clustering) |>
  plot_clusters_radars()
...
if (params$save_figures) {
  # ggsave(here("figures/cluster-radar.png"), dpi = 600)
  ggsave(
    filename = here("figures/cluster-radar.pdf"), 
    device = "pdf",
    width = 180,
    height = 80,
    units = "mm",
