...
cluster_results <- 
  tibble(
    # We compute the clustering on two datasets
    name        = c(
      # "scaled", 
      "reduced"
      ),
    df_to_clust = list(
      # Option 1: simple variable scaling --------------------------------------
      # df |> 
      #   scale_vars() |> 
      #   select(vviq:span_digit, score_similarities),
      # Option 2: reduction based on partial correlations ----------------------
      df |>
        scale_reduce_vars() |>
...
df2 <- align_clusters(cluster_results, df)

df2 |>
  group_by(cluster, group, subcluster) |>
  reframe(
    n = n(),
    across(where(is.numeric), ~ mean(., na.rm = TRUE) |> round(2))
  ) |> 
  relocate(score_raven, score_sri)
```
