...
df_meta <- 
  ...
  select(Duration, Median, MAD, everything())

# raw data
df0 <- 
  read_xlsx("data/data-n96.xlsx") |>  
  select(
    id, age, sex, group, 
  ...
  # recoding factors
  mutate(
    group = factor(group, levels = c("Phantasic", "Aphantasic")),
  ...
  mutate(across(c(
    sex:field, occupation,
    field_code, occupation_code
    ), as.factor)) |>
  # everything else as numeric
  mutate(across(c(
    age, 
    # field_code, occupation_code, 
    vviq:score_comprehension), as.numeric))
