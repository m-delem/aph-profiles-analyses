...
- The very first line of code checks if the packages in your project are up to
  date (they are not) and runs `renv::restore()` if needed to install all the 
  necessary packages. You should have a prompt in the console asking you if you 
  want to do just that. Accept!

Then all the chunks below will run in sequence. First all the packages will
be downloaded and loaded automatically by `pacman`. Then computations will run, 
Bayesian models should be saved in the repo and will be loaded quickly 
...
if (!renv::status()$synchronized) renv::restore()
